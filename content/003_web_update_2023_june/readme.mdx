...
{ (async (x) => {return <img src={(await x)?.default?.src}/>})(import("./01.png")) }
...
{ (async (x) => {return <img src={(await x)?.default?.src}/>})(import("./02.png")) }

- App will now show if resolver is coming from a wildcard lookup

{ (async (x) => {return <img src={(await x)?.default?.src}/>})(import("./03.png")) }
