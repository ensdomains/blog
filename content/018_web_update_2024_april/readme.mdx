...
We've been super hard at work since our last update in August 2023.
Our blog may have taken a hiatus, but the updates kept coming so this will be a long post.
Grab your favorite caffeinated beverage and get comfortable.
...
After many long power sessions, the ENS team has finally put the finishing touches on our new documentation site. Take some time and review it. We've even included some of our event videos inside the doc portal.
...
| Buttons | Removed the extend and reminder buttons on the Ownership tab of expired names. |
...
End to End (E2E) testing is probably the biggest pain point in front-end development, given that this involves spinning up a browser which is a relatively heavy piece of software, as well as the need to spin up any other required supporting infrastructure (ex. databases).
This is complicated even further in web3 as we have wallet interactions and their interactions with the blockchain to add into the mix.
They tend to be notoriously flaky and it can be hard to keep tests isolated as to achieve this you need to spin up a completely new environment for every test run.
All of these things and more add up and severly slow down the time between the code being ready and it getting deployed into production.
